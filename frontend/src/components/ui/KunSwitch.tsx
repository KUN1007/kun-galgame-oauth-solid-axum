...
export const KunSwitch: Component<KunSwitchProps> = (props) => {
  ...
  return (
    <label class={cn('inline-flex cursor-pointer items-center', props.disabled ? 'cursor-not-allowed' : '', props.class)}>
      <input
        id={id}
        type="checkbox"
        class="peer sr-only"
      ...
      {props.label && (
        <span class={cn('ml-3 text-sm font-medium', props.disabled ? 'text-default-400' : '', props.labelClass)}>
          {props.label}
        </span>
      )}
    </label>
